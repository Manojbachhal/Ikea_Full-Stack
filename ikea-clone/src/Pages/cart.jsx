...
const Cart = () => {
  ...
  const updateDeleteData = async (cartItem) => {
    ...
    let token = JSON.parse(localStorage.getItem("Token"));
    let res = await axios.post(
      `https://ikea-backend.onrender.com/products/cart/remove`,
      {
        cartItem,
        token,
      },
  ...
  const cartDelete = async (cartItem) => {
    ...
    let token = JSON.parse(localStorage.getItem("Token"));
    let res = await axios.post(
      `https://ikea-backend.onrender.com/products/cart/remove-product`,
      {
        cartItem,
        token,
      }
                ...
                return (
                  <div className="cartdivmain">
                    ...
                    <div className="cartdetails">
                      ...
                      <h5>
                        {ele.salesPrice_prefix} {ele.salesPrice_numeral}
                      </h5>
                      <div className="cartproduct">
                        <div className="cartproductbutton">
                          <button onClick={() => updateData(ele)}>
                            <AiOutlinePlus />
                          </button>
                          <button>{ele.quantity}</button>
                          <button
                            disabled={ele.Qty == 1}
                            onClick={() => updateDeleteData(ele)}
                          >
                            <BiMinus
        ...
        <div className="cartempty">
          <div class="cartemptyinner">
            ...
            <button
              type="button"
              class=""
              aria-label="Open context menu for shopping bag"
        ...
        <div className="cartemptygo">
          <div class="cartemptyinnergo">
            <div>
              ...
              <span>
                <a href="">Join or log in</a>for a smoother checkout
              </span>
            </div>
            <span>
              <i className=""></i>
